...
基于 AI 的面试练习平台。用户选择岗位、难度和岗位需求描述后，系统生成定制题目，并支持逐题多轮追问、评分、综合反馈、题库练习、Skill 驱动出题和多模型配置。受控 Agent 2.0 可在冻结题量、角色和评分规则的前提下动态规划只读工具、按证据缺口反思后续问题，并提供不含思维链的活动审计；v1/v2 会话可并存恢复。
