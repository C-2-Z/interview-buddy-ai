...
export const createInterviewSession = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const backgroundInfo = data.background?.trim() 
      ? `\n岗位需求描述: ${data.background}`
      : "";

    const prompt = `你是一位资深的技术面试官。请为以下岗位的招聘生成 ${data.questionCount} 道面试题。

岗位: ${data.position}
难度: ${data.difficulty}${backgroundInfo}
...
export const evaluateAnswer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: q, error } = await supabase
      ...
      .select("id, question, session_id, interview_sessions(position, difficulty, background)")
    ...
    const prompt = `作为面试官，请参考岗位需求评估以下回答：
...
2. feedback: 详细的评价与改进建议（200-400字，包含优点、不足、改进建议，评估时要参考岗位需求）
