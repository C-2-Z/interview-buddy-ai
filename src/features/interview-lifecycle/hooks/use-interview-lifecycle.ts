...
export function useInterviewLifecycle(sessionId: string) {
  ...
  const transition = useCallback(
    async (action: InterviewLifecycleAction): Promise<InterviewLifecycleResult> => {
      setPending(action);
      setError(null);
      try {
        return await transitionInterviewSession(sessionId, action);
      } catch {
        setError("操作未完成，请重试；若问题持续存在请联系管理员。");
  ...
  const remove = useCallback(async () => {
    ...
    setError(null);
    try {
      return await deleteInterviewSession(sessionId);
    } catch {
      setError("删除未完成，请重试或联系管理员。");
      throw new Error("lifecycle_delete_failed");
    } finally {
      setPending(null);
    }
  }, [sessionId]);
