...
/** 关联文档到知识库 */
export function addDocumentsToBrain(id: string, params: AddDocumentsToBrainParams): Promise<{ success: boolean }> {
  return apiRequest("POST", `/api/knowledge/brains/${id}/documents`, params);
}

/** 从知识库移除文档 */
export function removeDocumentFromBrain(brainId: string, documentId: string): Promise<{ success: boolean }> {
...
export function deleteDocument(id: string): Promise<{ success: boolean }> {
 ...
 /** 批量删除文档 */
 export function batchDeleteDocuments(ids: string[]): Promise<{ success: boolean; deletedCount: number }> {
...
export function askQuestionStream(
  ...
  callbacks: {
    ...
    onMeta: (data: { citedChunks: Array<{ chunkId: string; documentId: string; content: string; similarity: number }>; fullAnswer: string }) => void;
  ...
  (async () => {
    ...
    fetch(`${apiBaseUrl}/api/knowledge/qa/sessions/${sessionId}/ask`, {
      method: "POST",
      headers,
