...
export function GraphExplorer() {
 ...
 const [d3Force, setD3Force] = useState<any>(null);
  ...
  const { data: backlinksData, isLoading: backlinksLoading } = useBacklinks(detailNode?.type === "chunk" ? detailNode.id : null, minSimilarity);
  ...
  useEffect(() => {
    ...
    const sim = forceSimulation(nodes)
      ...
      .force("collide", forceCollide().radius((d: any) => d.size + 4));
  ...
  useEffect(() => {
    ...
    renderGraph(canvasRef.current, nodesRef.current, linksRef.current, transformRef.current, interaction.hoveredNode);
  ...
  function renderGraph(
    ...
    for (const node of nodes) {
      ...
      const isSelected = interaction.selectedNode?.id === node.id;
  ...
  function handleCanvasClick(e: React.MouseEvent<HTMLCanvasElement>) {
    ...
    if (closest) {
      ...
      if (graphNode?.type === "chunk") {
        setSelectedChunkId(graphNode.id);
      }
    } else {
