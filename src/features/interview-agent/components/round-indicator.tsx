 ...
 import type { AgentSnapshot, AgentRoleId, AgentWorkspace, AgentWorkspaceMessage } from "../types";
 ...
 /** 根据追问次数推算当前轮次类型。 */
 function inferRoundType(followUpCount: number, messages: AgentWorkspaceMessage[]): QuestionRoundType {
 ...
 export function RoundIndicator({
   ...
   const messages = currentQuestion?.messages ?? [];
 
   const roundType = useMemo(
     () => inferRoundType(snapshot.followUpCount, messages),
     [snapshot.followUpCount, messages],
   );
