...
/** 读取页面恢复投影。 */
export function getAgentWorkspace(sessionId: string): Promise<AgentWorkspace> {
...
export function connectAgentVoice(
  sessionId: string,
): Promise<{ wsUrl: string; expiresAt: string }> {
  return apiRequest("POST", `/api/agent/sessions/${sessionId}/voice/connect`, {});
}
