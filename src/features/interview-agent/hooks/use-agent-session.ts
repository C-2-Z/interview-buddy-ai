...
export function useAgentSession(initialSessionId?:string):UseAgentSessionResult{
  ...
  const applyEvent=useCallback((event:AgentSSEEvent)=>{cursorRef.current=Math.max(cursorRef.current,event.sequence);setLastEvent(event);if(event.type==="agent.snapshot")setWorkspace((current)=>current?{...current,snapshot:event.data}:current);else if(event.type==="agent.phase")setWorkspace((current)=>current?{...current,snapshot:{...current.snapshot,phase:event.data.phase}}:current);else if(event.type==="agent.question_ready")setWorkspace((current)=>current?{...current,questions:[...current.questions.filter((item)=>item.id!==event.data.id),{...event.data,score:null,feedback:null,messages:[],evidence:[],evaluation:null}].sort((left,right)=>left.orderIndex-right.orderIndex)}:current);else if(event.type==="agent.activity")setWorkspace((current)=>current?{...current,activities:[...current.activities.filter((item)=>item.id!==event.data.id),event.data].slice(-20)}:current);if(["agent.message_completed","agent.score_completed","agent.session_completed"].includes(event.type)||(event.type==="agent.activity"&&event.data.status!=="running"))void refresh();},[refresh]);
