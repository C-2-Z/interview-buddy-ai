...
export function useAgentSession(initialSessionId?:string):UseAgentSessionResult{
  ...
  const applyEvent=useCallback((event:AgentSSEEvent)=>{cursorRef.current=Math.max(cursorRef.current,event.sequence);setLastEvent(event);if(event.type==="agent.snapshot")setWorkspace((current)=>current?{...current,snapshot:event.data}:current);else if(event.type==="agent.phase")setWorkspace((current)=>current?{...current,snapshot:{...current.snapshot,phase:event.data.phase}}:current);else if(event.type==="agent.activity")setWorkspace((current)=>current?{...current,activities:[event.data,...current.activities.filter((item)=>item.id!==event.data.id)].slice(0,20)}:current);if(["agent.question_ready","agent.message_completed","agent.score_completed","agent.session_completed","agent.activity"].includes(event.type))void refresh();},[refresh]);
