...
import { InterviewLifecycleService, InterviewLifecycleServiceError } from "./interview-lifecycle.service.js";
...
function createRepository(overrides: Partial<InterviewLifecycleRepository> = {}): InterviewLifecycleRepository {
  return {
    async transition(sessionId, action) {
      return {
        sessionId,
        status: action === "finish" ? "completed" : action === "abandon" ? "abandoned" : action === "pause" ? "paused" : "in_progress",
...
test("finish commits partial report and clears checkpoint", async () => {
  ...
  const service = new InterviewLifecycleService({
    ...
    async deleteCheckpoint(threadId) { cleared.push(threadId); },
  });
...
test("repository errors become stable recovery errors", async () => {
  const service = new InterviewLifecycleService({
    repository: createRepository({
      async transition() { throw new Error("raw database password and stack"); },
    }),
  });
  await assert.rejects(
    service.transition(SESSION_ID, "resume"),
    (error: unknown) => error instanceof InterviewLifecycleServiceError
