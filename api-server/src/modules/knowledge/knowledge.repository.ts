...
): Promise<Array<{ chunkId: string; documentId: string; content: string; similarity: number; documentTitle: string }>> {
...
function mapQaMessageRow(row: Record<string, unknown>): QaMessage {
  return {
    ...
    citedChunks: (row.cited_chunks as Array<Record<string, unknown>>)?.map((c) => ({
