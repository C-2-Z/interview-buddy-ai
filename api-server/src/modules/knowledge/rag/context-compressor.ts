...
export function compressResults(
  ...
  // 阈值代表调用方明确的相关性边界；无命中时应走无上下文回答，不能重新注入低相关资料。
  if (filtered.length === 0) {
    logger.info("所有结果低于相似度阈值，返回空上下文");
    return { context: "", compressedResults: [] };
  }
  ...
  const truncated = filtered.map((r) => ({
    ...r,
    content:
      r.content.length > cfg.maxChunkChars
  ...
  const context =
    kept.length > 0
      ? header +
        kept.map((r, i) => `[${i + 1}] 来自《${r.documentTitle}》\n${r.content}`).join("\n\n")
      : "";
...
export function buildCompressedCitedChunks(results: SearchResult[]): Array<{
  chunkId: string;
  documentId: string;
  content: string;
  similarity: number;
