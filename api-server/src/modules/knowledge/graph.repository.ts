...
export async function getBacklinks(
  ...
  // 获取文档标题
  const docIds = [
    ...new Set((chunks ?? []).map((c: Record<string, unknown>) => c.document_id as string)),
  ];
...
export async function assembleGraphData(
  ...
  for (const chunk of chunkList) {
    ...
    nodes.push({
      ...
      label:
        (chunk.content as string).length > 30
      ...
      size: 6,
      content:
        (chunk.content as string).length > 200
...
export async function deleteAllEdgesByUser(
  ...
  const { error } = await supabase.from("knowledge_graph_edges").delete().eq("user_id", userId);
