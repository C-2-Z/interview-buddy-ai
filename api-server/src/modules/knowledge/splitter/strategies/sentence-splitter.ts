      ...
      for (const sentence of sentences) {
        ...
        if (sentence.length > chunkCharSize) {
          if (buffer.trim().length > 0) {
            chunks.push({ content: buffer.trim(), index: chunkIndex++, tokenCount: this.estimateTokens(buffer) });
            buffer = "";
          }
          chunks.push({ content: sentence, index: chunkIndex++, tokenCount: this.estimateTokens(sentence) });
          continue;
        }
