...
export class SplitterRegistry {
  private strategies = new Map<SplitterStrategy, new (config?: Partial<SplitterConfig>) => SplitterBase>();

  /** 注册一个分块策略 */
  register(name: SplitterStrategy, ctor: new (config?: Partial<SplitterConfig>) => SplitterBase): void {
  ...
  getInstance(name: SplitterStrategy, config?: Partial<SplitterConfig>): SplitterBase {
    ...
    if (!ctor) {
      throw new Error(`未知的分块策略: ${name}。可用策略: ${Array.from(this.strategies.keys()).join(", ")}`);
    }
