...
import {
  getGraphData,
  getBacklinksForChunk,
  rebuildAllGraphEdges,
...
knowledge.post("/documents/batch-delete", async (c) => {
  ...
  if (!ids || !Array.isArray(ids) || ids.length === 0) {
    return c.json({ error: "请选择要删除的文档" }, 400);
  }
  for (const id of ids) {
...
knowledge.post("/qa/sessions/:id/ask", async (c) => {
  ...
  return stream(c, async (s) => {
    ...
    try {
      ...
      logger.error("SSE 流式提问失败", err);
      await s.write(`data: ${JSON.stringify({ type: "error", content: err instanceof Error ? err.message : "未知错误" })}\n\n`);
    }
  });
});
...
knowledge.post("/search", async (c) => {
  const { query, documentIds, topK } = await c.req.json();
