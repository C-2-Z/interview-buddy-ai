...
agentReadinessRoutes.onError((error,context)=>{
  if(error instanceof z.ZodError)return context.json({error:"检查参数无效",code:"readiness_invalid_request"},400);
  logger.error(new Error("Agent readiness check failed"),{method:context.req.method,path:context.req.path});
  return context.json({error:"暂时无法完成开始前检查",code:"readiness_check_failed",retryable:true},503);
});
