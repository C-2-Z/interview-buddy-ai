...
/** 构造可被严格契约校验的完整工作台投影。 */
function buildWorkspace(version: "agent-v1" | "agent-v2" = "agent-v2"): AgentWorkspace {
  return {
    ...
    snapshot: {
      ...
      eventCursor: 8,
      ...(version === "agent-v2" ? { strategyRevision: 1 } : {}),
    },
    config: {
      position: "后端工程师",
      difficulty: "高级",
...
test("workspace restores the full projection with one database RPC", async () => {
  ...
  const database: WorkspaceDatabaseClient = {
    rpc(name, parameters) {
      rpcCalls += 1;
      assert.equal(name, "get_agent_workspace");
...
test("v1 workspace remains valid without v2 strategy or activities", async () => {
  const workspace = buildWorkspace("agent-v1");
  ...
  assert.equal(restored.snapshot.version, "agent-v1");
