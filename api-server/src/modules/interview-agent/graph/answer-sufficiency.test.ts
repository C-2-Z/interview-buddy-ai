...
test("short or abstract answers expose a stable first evidence gap", () => {
  assert.deepEqual(assessAnswerSufficiency("我优化了系统。"), {
    sufficient: false,
    gap: "too_brief",
  });
  assert.equal(
    assessAnswerSufficiency(
      "这个项目的整体情况比较复杂，团队需要综合考虑架构、资源、交付周期、业务价值和长期风险之后才能做出合适判断与取舍。",
    ).gap,
    "missing_action",
  );
});
...
test("concrete action, result and metric reaches minimum sufficiency", () => {
  const answer =
    "我先分析慢查询日志和执行计划，定位到联合索引缺失；随后调整索引并进行压测，最终接口 P95 延迟从 800 毫秒降低到 120 毫秒。";
...
test("extractKeywords ignores latency percentiles", () => {
  assert.deepEqual(extractKeywords("压测后 P95 从 800 毫秒降低到 120 毫秒，性能达到预期。"), []);
});
